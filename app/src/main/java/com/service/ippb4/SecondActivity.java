...
public class SecondActivity extends  BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ids = new HashMap<>();
        ids.put(R.id.newmpin, "newmpin");
        ids.put(R.id.reentermpin, "reentermpin");
        ...
        buttonSubmit.setOnClickListener(v -> {
            ...
            EditText newMpin1 = findViewById(R.id.newmpin);
            EditText reenterMpin1 = findViewById(R.id.reentermpin);
            String newMpin = newMpin1.getText().toString();
            String reenterMpin = reenterMpin1.getText().toString();
            if (!newMpin.equals(reenterMpin)) {
                Toast.makeText(this, "Please enter same MPIN in both fields", Toast.LENGTH_SHORT).show();
                return;
            }
            submitLoader.show();
            try {
    ...
    public boolean validateForm() {
        ...
        for (Map.Entry<Integer, String> entry : ids.entrySet()) {
            ...
            switch (key) {
                case "reentermpin":
                case "newmpin":
                    if (!FormValidator.validateMinLength(editText, 4, "Invalid MPIN")) {
                        isValid = false;
                    }
                    break;
                default:
